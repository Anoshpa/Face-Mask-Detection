...
# This function takes an image as input and passes it through faceNet first to extract a face to predict depening on
# the confidence score threshold and then passing that face through the model to predict if a mask is observed or not
# and then return the final result with the face detection outline drawn with green or red color and the confidence
# score mentioned as well
...
# This function is used to load the saved model for face mask detection(our model) as well as the model for face
# detection(faceNet) and returns it
def getModels():
...
# This function takes the input image and passes it through faceNet model to detect faces and return the detections
